...
// Community login — BLUE theme. Admin / official member emails are rejected
// with a clear "wrong login type" message.
export default function CommunityLoginPage() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (type.type === 'official_member') {
      setError('This account belongs to an Official Member. Please use the Member Login.')
      setLoading(false)
      return
    }
    if (type.type === 'none') {
      setError('Community account भेटिएन — पहिले Community signup गर्नुहोस्।')
      setLoading(false)
      return
    }
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-blue-50/70 px-5 py-10">
      <form onSubmit={submit} className="flex w-full max-w-md flex-col gap-5 rounded-3xl border border-blue-200 bg-card p-7 shadow-2xl shadow-blue-200/50">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-blue-600">Join community</p>
          <h1 className="mt-3 font-serif text-4xl font-bold">Community login</h1>
        </div>
          ...
          <div className="rounded-2xl border border-blue-300 bg-blue-50 p-4">
            {codeStatus !== 'verified' ? (
              <>
                ...
                {codeStatus === 'sent' && (
                  <>
                    <input value={code} onChange={(e) => setCode(e.target.value)} placeholder="६ अंकको code" inputMode="numeric" maxLength={6} className="mt-1 w-full rounded-xl border bg-background px-3 py-2.5 text-center text-lg font-bold tracking-[0.4em] outline-none focus:border-blue-500" />
                    ...
                    <button type="button" onClick={getCode} className="mt-2 w-full text-center text-xs font-semibold text-blue-700 underline">Code आएन? फेरि पठाउनुहोस्</button>
                  </>
                )}
              </>
            ) : null}
            {info && <p className="mt-2 text-xs leading-5 text-blue-800">{info}</p>}
          </div>
        )}
        <button disabled={loading} className={primary}>{loading ? 'Signing in…' : 'Community login'}</button>
        <a href="/community-signup" className="text-center text-sm font-semibold text-blue-700">New here? Join our community</a>
